...
for idx in X_test[m2m_mask].index:
    ...
    blanket_lifts.append(max(0.0, p_o - p_m))
...
ax2.set_ylabel("Median ROI (Jahr-1-Basis)", fontsize=11)
...
clv_gerettet     = retained_kunden * CLV
...
kpis = [
    ...
    ("Nettonutzen (CHF)",
     ...
     "CLV-Gewinn abzüglich Rabattkosten"),
    ("ROI (Jahr-1-Basis)",
...
for ax, seg, col in zip(axes, ["High Risk", "Medium Risk", "Low Risk"],
    ...
    ax.set_xlabel("Rabattkosten / Jahr (CHF)", fontsize=10)
    ax.set_ylabel("Erwarteter CLV-Gewinn (CHF)", fontsize=10)
    ax.set_title(f"{seg}\n(Top-30 nach ROI, n_ges={len(sub[sub.discount_id != 'none'])})",
                 fontsize=10.5, fontweight="bold", color=col)
    plt.colorbar(scatter, ax=ax, label="Retention-Lift", format="{x:.0%}")
    ax.spines[["top", "right"]].set_visible(False)

fig.suptitle("ROI-Profil: Kosten vs. CLV-Gewinn (Punktgrösse = Churn-Score)\n"
...
print(f"Targeted ROI:     {targeted_roi:.1%}  (Jahr-1-Basis)")
print(f"Blanket Budget:   CHF {blanket_budget:,.0f}")
print(f"Blanket Retained: {blanket_retained:.1f}")
print(f"Blanket ROI:      {blanket_roi:.1%}  (Jahr-1-Basis)")

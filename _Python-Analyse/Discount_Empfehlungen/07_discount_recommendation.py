...
CLV = 2_000.0   # Customer Lifetime Value in CHF
...
DISCOUNTS = {
    "contract_1y": {
        ...
        "cost_fn":     lambda row: 0.10 * row["MonthlyCharges"] * 12,
    ...
    "contract_2y": {
        ...
        "cost_fn":     lambda row: 0.15 * row["MonthlyCharges"] * 12,
    ...
    "payment_switch": {
        ...
        "cost_fn":     lambda _: 60.0,   # 5 CHF/Monat × 12
    ...
    "support_bundle": {
        ...
        "cost_fn":     lambda row: 0.08 * row["MonthlyCharges"] * 12,
    ...
    "senior_care": {
        ...
        "cost_fn":     lambda row: 0.12 * row["MonthlyCharges"] * 12,
...
def evaluate_customer(row_series, p_orig):
    ...
    for disc_id, disc in DISCOUNTS.items():
        ...
        clv_saved = lift * CLV
        roi   = (clv_saved - cost) / cost if cost > 0 else 0.0
        results.append({
            "discount_id":    disc_id,
...
print(f"{'Erwarteter CLV-Gewinn (CHF)':<35} {blanket_clv_saved:>10,.0f} {targeted_clv_saved:>12,.0f}")
...
with PdfPages(pdf_path) as pdf:
    ...
    for page_num, (cust_idx, (title, subtitle)) in enumerate(
        ...
        kpi_items = [
            ...
            ("Erwartete CLV-\nEinsparung", f"CHF {rec['clv_saved']:.0f}", C_BLUE),
        ...
        interp = (
            ...
            f"(Retention-Lift: {lift:.1%}). Bei einem Customer Lifetime Value von CHF {CLV:,.0f} "
            f"entspricht dies einem\n"
            f"erwarteten Einsparungspotenzial von CHF {rec['clv_saved']:,.0f} – bei jährlichen "
            f"Rabattkosten von CHF {rec['cost_annual']:.0f}.\n"

...
HORIZON_MONTHS = 24  # Planungshorizont fuer ROI-Berechnung
...
DISCOUNT_META = {
    ...
    "payment_switch": {"label": "Kreditkarten-Umstieg",             "color": M_ORANGE, "cost": "CHF 5/Monat × 24 Mt."},
...
def chart_blanket_vs_targeted():
    categories = ["Erwartete\ngehaltene Kunden", "Umsatz gerettet\n(CHF)", "Nettonutzen\n(CHF)"]
...
alg_lines = [
    ...
    "      e. ROI = (retention_lift × MonatsGebühr × 24 − cost) / cost",
...
story.append(formula("ROI = (Retention_Lift × MonatsGebühr × 24 − Rabattkosten) / Rabattkosten"))
story.append(body(
    "wobei <b>Retention_Lift</b> die Wahrscheinlichkeit angibt, mit der ein Kunde "
    "durch das Angebot gehalten wird, und <b>MonatsGebühr × 24</b> den geretteten "
    "Umsatz über einen 24-monatigen Planungshorizont darstellt. Das System wählt stets "
    "den Rabatt mit dem höchsten ROI – also maximale Wirkung bei minimalen Kosten."
))
story.append(box(
...
for cust_idx, section_title, disc_id, fig_label in examples_config:
    ...
    story.append(body(
        ...
        f"| Kosten (24 Mt.): CHF {rec['cost_annual']:.0f} "
        f"| Retention Lift: {rec['retention_lift']:.1%} "
        f"| Umsatz gerettet: CHF {rec['revenue_saved']:.0f} "
        f"| ROI: {rec['roi']:.0%}"
    ))
...
comp_tbl = Table([
    ...
    ["Umsatz gerettet (24 Mt., CHF)", f"CHF {blanket_clv_saved:,.0f}", f"CHF {targeted_clv_saved:,.0f}"],
